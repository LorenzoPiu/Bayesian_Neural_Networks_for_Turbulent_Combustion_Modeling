...
model_folder = build_model_folder(architecture='BNN',
                                  input_dim=input_dim,
                                  n_neurons=n_neurons,
                                  n_layers=n_layers,
...
if os.path.exists(os.path.join(model_folder, 'model.pth')):
    ...
    # Loss function
    def loss_function(output, target, kl_divergence, kl_weight=1.0):
    ...
    for epoch in range(n_epochs):
        ...
        for idx in ordered_indices_generator(len(data['x_train']), batch_size):
            ...
            # Forward pass
            output = bnn.forward(X_batch)
            # output, _ = bnn.forward_MC(X_batch, n_samples=5)
            
            # Compute KL divergence
            kl_div = bnn.kl_divergence()
            
            loss = loss_function(output, Y_batch, kl_div, kl_weight=kl_weight)
        ...
        if (('x_val' in data) and ('y_val' in data)):
            ...
            for idx in ordered_indices_generator(len(data['x_val']), batch_size):
                ...
                with torch.no_grad():
                    # Forward pass + loss
                    outputs = bnn.forward(X_batch)
                    test_loss = loss_function(outputs, Y_batch, kl_div, kl_weight=kl_weight)
...
bnn.eval()
with torch.no_grad():
    y_model_mean, y_model_sigma = bnn.forward_MC(x_lin)
